...
if(isset($_GET['action']) && ($_GET['action'] === 'edit' || $_GET['action'] === 'delete') || $_GET['action'] === 'editor') {
	...
	$query = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$tblGroup} WHERE `compiler_id` = {$_GET['id']} LIMIT 1"));
	if(count($query) == 0) {
		wp_redirect('./');
	} else {
		$actionItem = $query[0];
		...
		<form action="<?php echo esc_attr( admin_url('admin-post.php') ); ?>" id="frm" method="post">
			...
			<?php } else if(isset($_GET['action']) && $_GET['action'] === 'editor') { ?>
				...
				<?php if(isset($_GET['showerror']) && $_GET['showerror'] !== '') {?>
					<div class="error">
						<pre><span><?php echo esc_html__('Something wrong at line', 'best-css-compiler' )?>: </span><font color="#cc0000">'<?php echo esc_attr(1 + (int)$_GET['showerror'])?>'</font> <i>(error=true)</i></pre>
					</div>
				<?php } ?>
				<div class="form-required term-name-wrap">
